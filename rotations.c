...
//          crystalNumber = gtinp->crystal_id % 4;
	      ...
	      if(gtinp->intpts[j].y<0) gtinp->intpts[j].y-=GTPosOffsetY1;
	      else gtinp->intpts[j].y-=GTPosOffsetY2;
      ...
      if (ptgd->type == GEB_TYPE_DECOMP)
        {
          ...
          if (Pars.nprint > 0)
            {
              fprintf (stderr,"** %2i:\n", crystalID);
              fprintf (stderr,"AG::Tr?: %10.5f %10.5f %10.5f\n", Pars.TrX[crystalID], Pars.TrX[crystalID], Pars.TrZ[crystalID]);
