...
//          if (gtinp->pad>=128) gtinp->pad-=128; // uncomment to include pad == 128 events
...
//                return (BADPAD); // comment this line to just get rid of bad pad cluster but still track the rest in the same event

                // BADPAD is the last gammaray event, just get rid of it
                if ( i == track->n - 1) {
                    track->n --;
                    continue;
                }

                // get rid of the BADPAD event
                int j = 0 ;
                for ( j = i; j < track->n - 1 ; j++) {
                    memcpy((char *) & (track->gd[j]), (char *) & (track->gd[j + 1]),
                           sizeof(GEBDATA));
                    memcpy((char *) & (track->payload[j]), (char *) & (track->payload[j + 1]),
                           track->gd[j + 1].length);
                }

                // decrease the size of current track structure by 1
                track->n -- ;
                // check updated new event on the same array position
                i -- ;
...
int
transferClusters (SHELLHIT *shellhit, CLUSTER_INTPTS Clstr[MAXCLUSTERHITS],
                  int *nClusters) {
...
//            printf("%3i,%3i: e=%7.1f --> ", i1,gtinp->intpts[j].seg,gtinp->intpts[j].e);
                gtinp->intpts[j].e = gtinp->intpts[j].e *
                                     Pars.SEGcal_gain[i1][gtinp->intpts[j].seg]
                ...
                if (gtinp->tot_e > 0 && Ineg == 0) {
                    for (j = 0; j < gtinp->num; j++) {
                        ...
                        r1 = gtinp->intpts[j].x * gtinp->intpts[j].x
                             + gtinp->intpts[j].y * gtinp->intpts[j].y + gtinp->intpts[j].z *
                             gtinp->intpts[j].z;
...
//                          shellhit->timestamp[k] = gtinp->timestamp*10+(unsigned long long) (gtinp->t0*10);
            ...
            if (i != j) {
                ...
                if (shellhit->relAng[j][i] < Pars.alpha[shellhit->nhit]) {
                    ...
                    fprintf (stderr, "%p:: %2i %2i; %9.6f, %9.6f\n", shellhit, i, j,
                             shellhit->relAng[i][j], Pars.alpha);
#endif
...
readChatFile (char *name) {
    ...
    while (pc != NULL) {
        ...
        if ((p = strstr (str, "echo")) != NULL) {
            ...
            if (fp1 == NULL) {
                ...
                while (st == 3) {
                    ...
                    fprintf (stderr, "CC det/offset/gain %3i %6.3f %9.6f\n", i1,
                             Pars.CCcal_offset[i1], Pars.CCcal_gain[i1]);
                    st = fscanf (fp1, "%i %f %f", &i1, &r1, &r2);
                };
            ...
            fprintf (stderr,
                     "will set fom to %5.2f for ndet==%i if outside %9.3f MeV to %9.3f MeV\n",
            ...
            nret = sscanf (str, "%s %i %f %f %f", str1, &i1, &Pars.singlehitrangeFOM,
                           &distToCrystal, &r3);
            CheckNoArgs (nret, 5, str);
            ...
            nret = sscanf (str, "%s %f %f %i %i %i", str2, &Pars.splitclusters_kickoutfom1,
                           &Pars.splitclusters_thresh1,
                           &Pars.splitclusters_minn1, &Pars.splitclusters_maxn1,
                           &Pars.splitclusters_maxtry1);
            CheckNoArgs (nret, 6, str);
            Pars.splitclusters1 = 1;
        } else if ((p = strstr (str, "splitclusters2")) != NULL) {
            nret =
                sscanf (str, "%s %f %f %i %i %i %f", str2, &Pars.splitclusters_kickoutfom2,
                        &Pars.splitclusters_thresh2,
                        &Pars.splitclusters_minn2, &Pars.splitclusters_maxn2,
                        &Pars.splitclusters_maxtry2,
                        &Pars.goodImproveFraction);
            CheckNoArgs (nret, 7, str);
            Pars.splitclusters2 = 1;
        } else if ((p = strstr (str, "recluster1")) != NULL) {
            nret =
                sscanf (str, "%s %f %f %i %i %f", str2, &Pars.recluster_kickoutfom1,
                        &Pars.recluster_thresh1,
                        &Pars.recluster_minn1, &Pars.recluster_maxtry1, &Pars.recluster_reduxfactor1);
            CheckNoArgs (nret, 6, str);
            Pars.recluster1 = 1;
        } else if ((p = strstr (str, "recluster2")) != NULL) {
            nret =
                sscanf (str, "%s %f %f %i %i %f", str2, &Pars.recluster_kickoutfom2,
                        &Pars.recluster_thresh2,
                        &Pars.recluster_minn2, &Pars.recluster_maxtry2, &Pars.recluster_reduxfactor2);
            CheckNoArgs (nret, 6, str);
            Pars.recluster2 = 1;
        } else if ((p = strstr (str, "combineclusters")) != NULL) {
            nret = sscanf (str, "%s %f %f %i %f", str2, &Pars.combineclusters_kickoutfom,
                           &Pars.combineclusters_thresh,
            ...
            nret = sscanf (str, "%s %f %f", str2, &Pars.matchmaker_kickoutfom,
                           &Pars.matchmakerMaxDist);
            CheckNoArgs (nret, 3, str);
            Pars.matchmaker = 1;
            ...
            nret = sscanf (str, "%s %f %f %f", str2, &Pars.checkElast_lo,
                           &Pars.checkElast_hi, &Pars.checkElast_penalty);
            ...
            nret = sscanf (str, "%s %f %f", str2, &Pars.checkEfirst_lo,
                           &Pars.checkEfirst_penalty);
            CheckNoArgs (nret, 3, str);
            Pars.checkEfirst = 1;
            ...
            if (i1 >= MAXSHELLHITS) {
                fprintf (stderr,
                         "error: max number if interaction points are %i [MAXSHELLHITS in ctk.h]\n",
                         MAXSHELLHITS);
...
//          fprintf (stderr,"Pars.alpha=%f rad %f deg\n", Pars.alpha, Pars.alpha / RAD2DEG);
    ...
    if (Pars.alpha[1] <= 0) {
        fprintf (stderr,
                 "you must at least specify the clustereing angle for %i interaction points, clusterangle %i xxx.xxx\n",
                 1,
                 1);
        exit (1);
    };
...
fixDoubleHitsInSegs (TRACK_STRUCT *track, STAT *ctkStat) {
    ...
    for (i = 0; i < track->n; i++) {
        if (ptgd->type == GEB_TYPE_DECOMP) {
            ...
            if (Pars.nprint > 0)
                for (j = 0; j < gtinp->num; j++) {
                    fprintf (stderr, "--> %6.2f %6.2f %6.2f, %8.2f; ", gtinp->intpts[j].x,
                             gtinp->intpts[j].y, gtinp->intpts[j].z,
                             gtinp->intpts[j].e);
            ...
            if (nbad == 1) {
                ...
                for (j = 0; j < gtinp->num; j++) {
                    if (seghit[gtinp->intpts[j].seg] == 2 && fp != NULL) {
                        fprintf (fp, "%6.2f %6.2f %6.2f, %8.2f; ", gtinp->intpts[j].x,
                                 gtinp->intpts[j].y,
                                 gtinp->intpts[j].z, gtinp->intpts[j].e);
            ...
            for (i = 0; i <= TOT_SEGS; i++)
                if (seghit[i] > 1) {
                    ...
                    if (Pars.fixdoublehitsCode == 0) {
                        ...
                        nn = gtinp->num;
                        for (j = 0; j < nn; j++) {
                            memcpy ((char *) &tmpintpts[j], (char *) &gtinp->intpts[j],
                                    sizeof (DCR_INTPTS));
                        };
                        ...
                        for (j = 0; j < nn; j++) {
                            if (seghit[tmpintpts[j].seg] == 1) {
                                memcpy ((char *) &gtinp->intpts[gtinp->num], (char *) &tmpintpts[j],
                                        sizeof (DCR_INTPTS));
                                gtinp->num++;
                            };
                        };
...
int
trackEvent (float
            target_pos[3],        /* external beam interaction position */
...
//                assert (Clstr[iCluster].fom < 150);
    ...
    for (itno = 0; itno < Pars.itterations; itno++) {
        ...
        if (Pars.recluster1) {
            ...
            if (Pars.nprint > 0)
                fprintf (stderr,
                         "track:trying to split untracked monster clusters using reCluster at ev # %i\n",
                         ctkStat->nEvents);
                ...
                if (Clstr[i].valid)
                    if (!Clstr[i].tracked)
                        if (Clstr[i].ndet >= Pars.recluster_minn1) {
                            i1 = -ctkStat->TrackingCalls;
                            st =
                                reCluster (i, ctkStat->nTrackCalled, ctkStat, Clstr, nClusters, target_pos,
                                           shellhit,
                                           Pars.recluster_reduxfactor1, Pars.recluster_maxtry1,
                                           Pars.recluster_kickoutfom1);
                            i1 += ctkStat->TrackingCalls;
        ...
        if (Pars.splitclusters1) {
            ...
            if (Pars.nprint > 0)
                fprintf (stderr,
                         "track: trying to split untracked monster clusters using splitCluster\n");
                ...
                if (Clstr[i].valid)
                    if (Clstr[i].fom > Pars.combineclusters_thresh)
                        if (Clstr[i].ndet <= Pars.combineclusters_maxn) {
                            i1 = -ctkStat->TrackingCalls;
                            st = combineCluster (i, ctkStat->nTrackCalled, ctkStat, Clstr, nClusters,
                                                 target_pos);
                            i1 += ctkStat->TrackingCalls;
                ...
                if (Clstr[i].valid)
                    if (Clstr[i].fom > Pars.recluster_thresh2)
                        if (Clstr[i].ndet >= Pars.recluster_minn2) {
                            i1 = -ctkStat->TrackingCalls;
                            st = reCluster (i, ctkStat->nTrackCalled, ctkStat, Clstr, nClusters,
                                            target_pos,
                    ...
                    if (Clstr[i].valid)
                        for (j = i + 1; j < numCurrentClusters; j++)
                            if (Clstr[j].ndet == 1)
                                if (Clstr[j].valid) {
                                    i1 = -ctkStat->TrackingCalls;
                                    st = matchMaker (i, j, ctkStat->nTrackCalled, ctkStat, Clstr, nClusters,
                                                     target_pos);
                                    i1 += ctkStat->TrackingCalls;
                ...
                if (Clstr[i].ndet == 1) {
                    ...
                    if (i1 > 0 && i1 < LONGLEN) {
                        r1 = Clstr[i].intpts[0].xx * Clstr[i].intpts[0].xx
                             + Clstr[i].intpts[0].yy * Clstr[i].intpts[0].yy + Clstr[i].intpts[0].zz *
                             Clstr[i].intpts[0].zz;
